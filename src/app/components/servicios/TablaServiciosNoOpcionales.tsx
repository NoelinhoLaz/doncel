...
import { Trash2, Plus, Link as LinkIcon, Download, Package } from "lucide-react";
...
function ImporteField({
  ...
  if (locked) {
    return (
      <div
        title={lockedReason}
        style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem", padding: "0.3rem 0.6rem", backgroundColor: "#f1f5f9", border: "1px solid #e2e8f0", borderRadius: "0.375rem", fontSize: "0.825rem", color: "#94a3b8", cursor: "not-allowed", minWidth: "90px" }}
      >
  ...
  if (editing) {
    return (
      <input
        ...
        style={{ width: "90px", padding: "0.3rem 0.5rem", border: "2px solid #3b82f6", borderRadius: "0.375rem", fontSize: "0.825rem", textAlign: "right", outline: "none", color: "#0f172a", fontWeight: 600 }}
      />
    );
  }
  ...
  return (
    <div
      ...
      style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem", padding: "0.3rem 0.6rem", backgroundColor: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: "0.375rem", minWidth: "90px", fontSize: "0.825rem", color: "#475569", cursor: "text", transition: "border-color 0.15s" }}
    ...
    <div className={styles.tabContainer} style={{ marginTop: "2rem" }}>
      ...
      <div className={styles.listHeaderTop} style={{ borderBottom: "1px solid #e2e8f0" }}>
        <div className={styles.listTitleWrapper}>
          ...
          <h2 className={styles.listTitle}>Servicios ({serviciosList.length})</h2>
        </div>
        <div className={styles.actionsWrapper}>
          <div style={{ position: "relative" }}>
            <button className={styles.addActionButton} title="Añadir servicio" onClick={() => setShowDropdown(!showDropdown)}>
              <Plus size={18} />
            </button>
            {showDropdown && (
      ...
      {serviciosList.length === 0 ? (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "3rem 1.5rem", backgroundColor: "#fff", borderRadius: "0.75rem", textAlign: "center", gap: "0.75rem" }}>
          <p style={{ fontSize: "0.85rem", color: "#64748b", margin: 0 }}>No hay servicios contratados importados en este listado.</p>
        </div>
      ) : (
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {serviciosList.map((item) => {
                ...
                // Non-optional services: lock if linked (traveler concept doesn't apply here — lock only if somehow blocked)
                const locked = false;
                const lockedReason = "No se puede editar el importe";
