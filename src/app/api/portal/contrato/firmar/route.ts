...
export async function POST(request: NextRequest) {
  try {
    ...
    const { expedienteId, screenResolution } = body;
    ...
    const firmaData = {
      ...
      userAgent,
      acceptLanguage,
      screenResolution: screenResolution || "Desconocida",
    };

    const { pdfBuffer, driveFileId } = await generateSignedContract(contractData, firmaData);
    ...
    return Response.json({ error: err.message || "Error al firmar contrato" }, { status: 500 });
  }
}
