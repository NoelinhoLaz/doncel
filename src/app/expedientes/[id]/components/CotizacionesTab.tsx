...
export default function CotizacionesTab({ expedienteId, hideHeader, compactHeader, hideSummary, onTotalsChange, title, cotizacionId, initialCotizacion, opcionalFilter, autoOpenSheetsImport, readOnly }: CotizacionesTabProps) {
  ...
  return (
    <>
      <div className={styles.tabContainer} style={{ overflow: 'visible', marginTop: compactHeader ? '-0.75rem' : '0px', borderRadius: '0.75rem' }}>
        <TablaCotizacion c={c} hideHeader={hideHeader} compactHeader={compactHeader} title={title} sidePanel={resumen} cotizacionId={cotizacionId} autoOpenSheetsImport={autoOpenSheetsImport} readOnly={readOnly} />
      </div>
