...
import { linkExpedienteDriveFolder } from "@/actions/expedientes";

interface DriveFolderPickerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onFolderLinked: (folder: { id: string; name: string }) => void;
  expedienteId: string;
}
...
export default function DriveFolderPickerModal({
  ...
  const loadDriveFolders = useCallback(async (parentId: string = "root") => {
    try {
      ...
      setFolderError(errorMsg || "Error al cargar carpetas.");
    } finally {
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : String(err);
      setErrorMsg(errorMsg || "Error al conectar con Google Drive");
    } finally {
      setLoading(false);
    }
  };
  ...
  const selectCurrentFolder = async () => {
    try {
      ...
      const res = await linkExpedienteDriveFolder(expedienteId, {
        id: current.id,
        name: current.name,
      });

      if (!res.success) {
        throw new Error(res.error || "Error al vincular la carpeta.");
      }

      setSuccessMsg(`✓ Carpeta vinculada: "${current.name}"`);
      onFolderLinked({ id: current.id, name: current.name });
      setTimeout(() => {
        setSuccessMsg(null);
        onClose();
      }, 1500);
      ...
      const errorMsg = err instanceof Error ? err.message : String(err);
      setFolderError(errorMsg || "Error al guardar la carpeta seleccionada.");
    } finally {
      setFolderLoading(false);
    }
  };
  ...
  const handleDisconnect = async () => {
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : String(err);
      setErrorMsg(errorMsg || "Error al desconectar Drive");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
            ...
            <div>
              <h2 style={{ fontSize: "1rem", fontWeight: "700", color: "#0f172a", margin: 0 }}>
                {isDriveConnected ? "Vincular Carpeta de Drive" : "Conectar Google Drive"}
              </h2>
              {isDriveConnected && (
                <p style={{ fontSize: "0.72rem", color: "#64748b", margin: "0.1rem 0 0 0" }}>
                  Navega y selecciona la carpeta para este expediente
                </p>
              )}
            </div>
          </div>
                ...
                {loading ? (
                  ...
                  <>
                    <svg width="16" height="16" viewBox="0 0 48 48">
                      <path
                        fill="#FFC107"
                        d="M43.6 20H24v8h11.3C33.7 33.4 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.9 1.2 8 3.1l5.7-5.7C34.2 6.5 29.4 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.7-.4-4z"
                      />
                      <path
                        fill="#FF3D00"
                        d="M6.3 14.7l6.6 4.8C14.5 15.1 18.9 12 24 12c3.1 0 5.9 1.2 8 3.1l5.7-5.7C34.2 6.5 29.4 4 24 4 16.3 4 9.7 8.3 6.3 14.7z"
                      />
                      <path
                        fill="#4CAF50"
                        d="M24 44c5.2 0 9.9-1.9 13.5-5l-6.2-5.2C29.4 35.3 26.8 36 24 36c-5.2 0-9.6-3.4-11.2-8.1l-6.6 5.1C9.8 39.6 16.4 44 24 44z"
                      />
                      <path
                        fill="#1976D2"
                        d="M43.6 20H24v8h11.3c-.8 2.3-2.3 4.2-4.3 5.5l6.2 5.2C41.1 35.3 44 30 44 24c0-1.3-.1-2.7-.4-4z"
                      />
                    </svg>
                    Conectar con Google
                  </>
                ...
                {folderPath.map((segment, i) => (
                  <span key={segment.id} style={{ display: "flex", alignItems: "center", gap: "0.25rem" }}>
                    ...
                    <button
                      ...
                      onMouseEnter={(e) => {
                        if (i < folderPath.length - 1) e.currentTarget.style.backgroundColor = "#e2e8f0";
                      }}
                      onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}
                    >
                      {segment.name}
              ...
              <div style={{ minHeight: "160px", maxHeight: "300px", overflowY: "auto" }}>
                {folderLoading ? (
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "2.5rem", gap: "0.5rem" }}>
                    ...
                    <span style={{ fontSize: "0.8rem", color: "#94a3b8" }}>Cargando carpetas...</span>
                  </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "2rem", color: "#94a3b8", gap: "0.5rem" }}>
                    ...
                    <span style={{ fontSize: "0.72rem" }}>Puedes vincular esta carpeta directamente</span>
                  </div>
                ) : (
        ...
        {isDriveConnected && !loadingConfig && (
          ...
          >
            ...
            <button
              ...
              onMouseEnter={(e) => {
                if (!isAtRoot) e.currentTarget.style.backgroundColor = "#e2e8f0";
              }}
              onMouseLeave={(e) => {
                if (!isAtRoot) e.currentTarget.style.backgroundColor = "#f1f5f9";
              }}
            >
            ...
            <button
              ...
              style={{
                ...
                opacity: folderLoading ? 0.7 : 1,
                transition: "background-color 0.2s",
              }}
              onMouseEnter={(e) => {
                if (!folderLoading) e.currentTarget.style.backgroundColor = "#059669";
              }}
              onMouseLeave={(e) => {
                if (!folderLoading) e.currentTarget.style.backgroundColor = "#10b981";
              }}
            >
              {folderLoading ? (
                <>
                  <Loader2 size={14} style={{ animation: "spin 1s linear infinite" }} /> Vinculando...
                </>
              ) : (
                <>
                  <Check size={14} /> Vincular carpeta &ldquo;{currentFolder.name}&rdquo;
                </>
              )}
            </button>
